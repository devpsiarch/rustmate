use super::defs::{SIDES,Pieces,Castle,UNICODE_PIECES,SQUARE_NAME,MAX_GAME_MOVES,MAX_MOVE_RULE,algb_to_square};
...
fn load_side_to_move(board :&mut Chessboard,part:&str) -> bool{
    ...
    board.side_to_move = match letter {
        'w' => SIDES::WHITE,
        'b' => SIDES::BLACK,
        _ => return false,
    };
...
fn load_en_passant(board :&mut Chessboard,part:&str) -> bool {
    ...
    if length == 2 {
        ...
        match square {
            Some(x) => {
                // I just noticed that my board is flipped relative to this
                // IDK if this will break later or not , Hope not
                board.en_passant = x;
                char_ok += 2;
            }
            None => (),
